...
{
    if (sbi_memcmp(file, ELFMAG, SELFMAG) != 0) {
        return -1;
    }
...
const void *elf_getSectionNamed(const elf_t *elfFile, const char *str, size_t *id)
{
    ...
    for (size_t i = 0; i < numSections; i++) {
        if (sbi_strcmp(str, elf_getSectionName(elfFile, i)) == 0) {
            if (id != NULL) {
                *id = i;
            }
...
int elf_loadFile(const elf_t *elf, elf_addr_type_t addr_type)
{
    ...
    for (i = 0; i < elf_getNumProgramHeaders(elf); i++) {
        ...
        sbi_memcpy((void *) dest, (void *) src, len);
        dest += len;
        sbi_memset((void *) dest, 0, elf_getProgramHeaderMemorySize(elf, i) - len);
    }
