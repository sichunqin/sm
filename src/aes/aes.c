...
* Details:    This code is the implementation of the AES algorithm and
              ...
              The CCM mode of operation is specified by NIST SP80-38 C,
available at:
...
/**************************** VARIABLES *****************************/
// This is the specified AES SBox. To look up a substitution value, put the
// first nibble in the first index (row) and the second nibble in the second
// index (column).
...
// This table stores pre-calculated values for all possible GF(2^8)
// calculations.This table is only used by the (Inv)MixColumns steps. USAGE: The
// second index (column) is the coefficient of multiplication. Only 7 different
// coefficients are used: 0x01, 0x02, 0x03, 0x09, 0x0b, 0x0d, 0x0e, but
// multiplication by 1 is negligible leaving only 6 coefficients. Each column of
// the table is devoted to one of these coefficients, in the ascending order of
// value, from values 0x00 to 0xFF.
...
void
xor_buf(const BYTE in[], BYTE out[], size_t len) {
  size_t idx;

  for (idx = 0; idx < len; idx++) out[idx] ^= in[idx];
}
...
int
aes_encrypt_cbc(
    const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize,
    const BYTE iv[]) {
 ...
 *******************/
void
increment_iv(BYTE iv[], int counter_size) {
  int idx;

  // Use counter_size bytes at the end of the IV as the big-endian integer to
  // increment.
  for (idx = AES_BLOCK_SIZE - 1; idx >= AES_BLOCK_SIZE - counter_size; idx--) {
    iv[idx]++;
    if (iv[idx] != 0 || idx == AES_BLOCK_SIZE - counter_size) break;
  }
}

// Performs the encryption in-place, the input and output buffers may be the
// same. Input may be an arbitrary length (in bytes).
void
aes_encrypt_ctr(
    const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize,
    const BYTE iv[]) {
...
SubWord(WORD word) {
  ...
  result += (int)aes_sbox[(word >> 12) & 0x0000000F][(word >> 8) & 0x0000000F]
            << 8;
  result += (int)aes_sbox[(word >> 20) & 0x0000000F][(word >> 16) & 0x0000000F]
            << 16;
  result += (int)aes_sbox[(word >> 28) & 0x0000000F][(word >> 24) & 0x0000000F]
            << 24;
  return (result);
}
...
// schedule, "keysize" is the length in bits of "key", must be 128, 192, or 256.
void
aes_key_setup(const BYTE key[], WORD w[], int keysize) {
  int Nb = 4, Nr, Nk, idx;
  WORD temp,
...
// Performs the AddRoundKey step. Each round has its own pre-generated 16-byte
// key in the form of 4 integers (the "w" array). Each integer is XOR'd by one
// column of the state. Also performs the job of InvAddRoundKey(); since the
// function is a simple XOR process, it is its own inverse.
void
AddRoundKey(BYTE state[][4], const WORD w[]) {
  BYTE subkey[4];
...
// multiplication in a Galios Field 2^8. All multiplication is pre-computed in a
// table. Addition is equivilent to XOR. (Must always make a copy of the column
// as the original values will be destoyed.)
void
MixColumns(BYTE state[][4]) {
  BYTE col[4];
...
aes_encrypt(const BYTE in[], BYTE out[], const WORD key[], int keysize) {
  ...
  // Copy input array (should be 16 bytes long) to a matrix (sequential bytes
  // are ordered by row, not col) called "state" for processing.
  // *** Implementation note: The official AES documentation references the
  // state by column, then row. Accessing an element in C requires row then
  // column. Thus, all state references in AES must have the column and row
  // indexes reversed for C implementation.
  state[0][0] = in[0];
  state[1][0] = in[1];
  state[2][0] = in[2];
  state[3][0] = in[3];
...
void
aes_decrypt(const BYTE in[], BYTE out[], const WORD key[], int keysize) {
  BYTE state[4][4];
...
// This prints the key (4 consecutive ints) used for a given round as a linear
hex string. void print_rnd_key(WORD key[])
{
  int idx;
